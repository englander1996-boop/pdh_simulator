...
最終更新: 2026-05-29 (命名統一・実在レポート14本・P10/FUG・各case内に欠陥部品節 / 夕: リポジトリ改名 special→main・main→sub1・final→sub2 と全参照更新 §0.5)
...
| D13 | **過去レポート著者の人名を repo (git) に残さない**。匿名 ID (case_rep_<テーマ><年><連番>) を使う。手元メモ (git外) は可 | ユーザ指示 (2026-05-29)。第三者氏名を公開 repo に残さない |
...
2. 初回推測「過去手法を再現したい」→ **ユーザ「違う」**。
3. 再推測「高度な最適化を誰も使っていない空白を立証し、自作 BO の新規性 (related work) を主張」
   → **ユーザ「半分正解」**。正確には「**こういう問題があるから BO を採用した。逆に過去の最適化手法を
   ここで再現することで BO の有用性を示す**」。
4. 2 つの詳細レポート (手法分析・問題点) を読了し構図を確認。
5. 「レポート抜粋_md があれば再現可能か?」→ 担当者「再現可能。ただし **📖最適化セクション原文だけを信じ、
...
6. ユーザ補足: 「軽い分析でよい。全部やると大変。**調査をもとにどれを再現するか決める。内包される問題が
   ...
   「**ファイルが大量なので構成も課題**」。
7. 担当者「**レポート単位でなく問題 (P) 単位で選ぶ**」方針 + 露呈度の表を提示。置き場所を質問
...
9. 担当者が本丸 `special.py` を読み、継ぎ目 (`SEARCH_SPACE` → `_build_design` → `evaluate`) を確認。
   backend を質問 → **ユーザ「本丸と同じ Dist2=HYSYS」** (D4)。
10. ディレクトリと初期ファイルを作成。**ここで 担当者が簡易自作の `io.py` を書いた → 却下** (下記)。
...
16. 担当者が `simulator.py` を special 流に作り直し、`reporting.py` / `harness.py` / `sequential_1d.py` /
...
19. 担当者が再編: 中央 `main.py` 廃止、FUG/smoke 全廃、欠陥部品 case を caseN=PN に整理、shared/ 流用維持。
...
5. **成熟度を散らす** (D15)、**著者名を書かない** (D13、匿名 ID。対応は 手元メモに)。
