...
# ---------------------------------------------------------------------------
# 活性炭に対する Langmuir 吸着定数 (25℃等温条件)
#
# ★★★ 仮置き値 — レポート提出前に一次文献値へ差し替えること ★★★
#
# 現状: 過去の学生レポートからの参考値であり学術的一次文献として不適格。
# 対応: 活性炭に対する CH4, C2H4, C2H6 の吸着等温線に関する学術論文を
#       調査し、q_s [mol/kg] および a [Pa⁻¹] を更新すること。
#
# データ構造:
#   q_s : 飽和吸着量 [mol/kg-adsorbent]
#   a   : Langmuir 親和定数（吸着平衡定数）[Pa⁻¹]
#         b(T) = a * exp(-ΔHads / (R*T)) の形で温度依存させる場合は拡張すること。
# ---------------------------------------------------------------------------
PSA_LANGMUIR_PARAMS: dict = {             # ★★★ 仮置き ★★★
    "CH4":  {"q_s": 7.06,    "a": 4.27e-3},   # [mol/kg], [m³/mol]
    "C2H6": {"q_s": 5.07,    "a": 1.12e-1},   # [mol/kg], [m³/mol]
    "C2H4": {"q_s": 2.72,    "a": 7.67e-2},   # [mol/kg], [m³/mol]
}
...
# ★★★ 仮置き値 — PDE ソルバー初期テスト用 ★★★
#
# 対応: Carberry 数や Knudsen 拡散係数を用いて、25℃・操作圧力・初期組成の
#       条件下で手計算または別スクリプトで算出した値に更新すること。
# ---------------------------------------------------------------------------
PSA_KFA: dict = {                          # ★★★ 仮置き ★★★
    "CH4":  0.5,   # [1/s]
    "C2H6": 0.8,   # [1/s]
