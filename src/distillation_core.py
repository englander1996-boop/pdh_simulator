...
Fenske-Underwood-Gilliland (FUG) 法ベースの簡易モデル。物理的に正しい温度・
熱量を与え、BO による P_col / N_stages / N_feed / reflux_ratio の最適化を可能にする。
...
# 各成分の大気圧沸点 [K] と蒸発潜熱 [kJ/mol] (PDH の主要成分の代表値)
_T_BOIL_ATM = {
...
class ColumnTunables:
    ...
    # recovery を BO で振れるよう field 化。
    # None → column1/2/3.py ラッパの既定値 (0.99) を採用。
    # float (e.g., 0.95〜0.999) → ラッパ既定値を上書き。
    recovery_LK_top: Optional[float] = None
    recovery_HK_bot: Optional[float] = None
    # D 直接指定 (HYSYS との比較・検証用)。
    # None → FUG が Fenske split で D を決定。
    ...
    # HYSYS の段数別 HSC ファイル + 主スペック書込み運用。
    ...
    # main の FUG/rigorous 経路では None のままで OK。
    hysys_spec_value: Optional[float] = None
    hysys_feed_stage: Optional[int] = None


# 分流 (partial condenser) で凝縮させない成分セット
# H2 (Tc=33K) と CH4 (Tc=190K) は工業的に到達可能な冷媒温度
# (-100°C エチレン = 173K) でも凝縮できないため、partial condenser では
...
# 分流 (partial condenser) で「液側に強く偏る」成分セット。
# C3 (Tc≈370K, P_sat@-30°C ≈ 1.5-1.6 bar) は Dist2 の操作圧 (7-8 bar) >> P_sat
# なので塔頂温度 -30〜-40°C で大半が凝縮する。
# ただし PR EOS 実値で K(C3H6) @ -43°C/7.5bar = 0.18 と「ゼロ」ではないため、
# 「閾値ベース丸め」を用いる: feed の _PARTIAL_COND_C3_ROUND_FRAC 未満の F_top 漏れだけ
...
# 方針「ほぼほぼ分離・有効数字範囲のバランス崩れは OK」。
_PARTIAL_COND_C3_ROUND_FRAC = 0.01

# rigorous プロキシ罰則の係数。FUG の楽観性を是正するため BO objective に乗せる。
# 単位は spec_coef_okuyen (= 100 億円/(年・%pt)) と同次元。
# narrow margin (R/R_min, N/N_min が小) や partial cond の C3 漏れ過大に罰則を与え、
# BO が FUG 楽観性領域に張り付くのを防ぐ。partial cond の塔は
# _compute_proxy_penalty 内で係数 ×2 を追加適用 (FUG 楽観性が特に大きいため)。
...
# partial cond / total cond で閾値を分ける:
#   partial cond (Dist2): FUG の non-key 分配が物理から大きく乖離するため厳しめ。
#   total cond (Dist1/Dist3): C3 splitter 等は physics 素直で実機運用 R/R_min=1.1-1.3
#                             が一般的なため緩め (厳しすぎると経済探索を阻害)。
# top-k で rigorous 再評価が走るため、BO loop の proxy は「FUG では feas、rigorous で
# 詰むかも」境界を TPE が探索できる程度に緩めに設定する。
_PROXY_R_MARGIN_THRESHOLD_PARTIAL = 1.30  # partial cond の R/R_min 閾値 (Dist2)
_PROXY_R_MARGIN_THRESHOLD_TOTAL   = 1.20  # total   cond の R/R_min 閾値 (Dist1/Dist3)
...
def _compute_proxy_penalty(
    ...
    *,
    ...
    # partial cond の塔 (Dist2) は FUG の non-key 分配が物理から大きく乖離しやすいので、
    # 係数を _PROXY_PARTIAL_COND_MULTIPLIER で底上げする。Dist1/Dist3 (total cond) は ×1.0。
    # margin 閾値も partial / total で分ける。
    ...
    # is_rigorous=True のときは leak 罰則をスキップ。
    ...
    # shortage は生値 (0-1) で扱う。threshold は partial / total で分岐
    # (上で r_threshold/n_threshold 選択済)。生値のまま coef 80 を掛けることで
    # BO best ~300 億スケールに対し数十億円オーダーの effective penalty を狙う。
    # partial cond は ×2 (上で coef_mult 適用済)。
...
class DistDesignVars:
    ...
    # D 直接指定モード (HYSYS の "Distillate Rate spec" 相当)。
    # None → FUG の Fenske split で D を決定。
...
class DistEquipment:
    ...
    # ---- infeasibility 連続シグナル (TPE constraints_func 用) ----
...
def _H_mol_jmol(T: float, P: float, comp_frac: Dict[str, float],
    ...
    partial condenser の凝縮器エネルギー収支 Q = V2·Hv − D·Hv − L1·Hl 用に
    厳密なエネルギー収支を組む (簡易な R×凝縮性成分×λ 式は塔頂蒸気の顕熱を
    無視して大幅に過小評価するため)。
    """
...
def _simulate_rigorous(
    ...
    K 値ライブラリの選択根拠:
      ...
      比較した結果、相対差 0.02% 以下で一致。新規依存を最小化するため
      src/eos.py を流用する。
    """
    ...
    # D_override がある場合は FUG の D を上書き (HYSYS の "Distillate Rate spec" 相当)。
    # 物理的に不適な値は solver 内で _failure_result 化。
    ...
    # ---- 失敗時 retry (Wegstein off + 低 damping) ----
    ...
    # MESH 残差 max > 0.05、成分バランス err > 5% を異常とみなす。
    # リサイクル過渡 (Δ_rel > 50% で組成が暴れている状態) では 1-2% の残差が
    # 普通に出るため、これより strict な閾値だと recycle 全体を止めてしまう。
    # 閾値超え分は proxy_penalty が捕捉する設計。
    _MESH_TOL = 0.05
    _MASS_BAL_TOL = 0.05
    ...
    # ---- recovery spec 達成度チェックは意図的に行わない ----
    # N=N_min ぎりぎりで rigorous が non-spec 解を返すケースの検出を狙うと、
    # 以下の理由で本来 OK な解も fallback してしまうため:
    ...
    # rigorous は物理的に正しく C3 を「ほぼほぼ塔底へ」振り分けるが、PR EOS 実値で
    # K(C3H6) ≈ 0.18 @ -43°C/7.5bar 程度の有限値があり、F_top に微小量 (数 kmol/h
    # オーダー) が残る。下流の PSA/Mem は組成上 C3 を流したくない設計なので、FUG パスと
    # 同じ閾値丸め (feed の _PARTIAL_COND_C3_ROUND_FRAC 未満なら 0 集約) を rigorous
    # 結果にも適用する。閾値超えは物理値を残して warning。
    # proxy_penalty 計算用に丸める前の F_top を保持する。
    ...
    if design.partial_condenser:
        # partial cond の Q_cond は凝縮器(stage1)の厳密エネルギー収支で計算する。
        # 収支: Q = V2·Hv(y2,T2) − D·Hv(y1,T1) − L1·Hl(x1,T1)。
    ...
    # FUG の冷媒 tier が温度未到達で ΔT 違反になりうるため、rigorous 結果で
    # utility を再選択して整合させる。
    ...
    # FUG と rigorous の値が異なるのでそれぞれで計算する。
    # 丸める前の F_top_pre_round を渡す (<1% 漏れも捕捉できる拡張余地を確保)。
...
def _supply_T_for_utility(utility_name: str) -> float:
    ...
    保持していないので、名前から tier テーブルの supply_T を再取得する。
    """
...
def simulate_distillation_column(
    ...
    """
    # ---- Solver dispatch ----
    # rigorous 指定時のフォールバックは fail-fast がデフォルト。例外を握り潰して
    # FUG にサイレント切替すると「表示は solver=rigorous なのに実体は FUG」となり
    # 結果に rigorous の物理が反映されない信頼性問題が起きる。fail-fast にして
    # 「rigorous 指定なら rigorous で通すか or 例外停止」を保証する。
    # BO 等で fallback が必要な場合は環境変数 PDH_RIGOROUS_STRICT=0 で warn + FUG
    # 挙動に切り替えられる退路を残す。
    if design.solver_method == 'rigorous':
        ...
        try:
            ...
            if _strict:
                # RuntimeError を raise すると run_one_pass で「Dist2 失敗 → 下流の
                # PSA/Mem が ValueError で crash」する経路があり、TPE constraints_func に
                # dT_max 等の連続シグナルが伝わらない。そのため penalty_result を返して
                # `equipment.feasible=False` でシグナル化し、`dT_max_rigorous` を抽出して
                # run_one_pass で user_attr に格納する。silent FUG fallback はしない。
                msg = str(e)
    ...
    T_bot = _boil_cc(_T_BOIL_ATM.get(design.HK, _T_BOIL_DEFAULT),
                     ...
                     design.P_col)
    # T_top は _T_COND_MIN で clamp しない。「冷却水到達下限 40°C」で上書きすると
    # Dist2 の x_top (H2/CH4 主体) で実際は cryogenic な凝縮器が必要なケースを
    # 隠蔽してしまうため、実温度を保持し utility_selector で適切な冷媒を選ぶ。
    ...
    # K_avg を「塔平均 T で x=z」で計算すると、この T-P-x は単相領域に入りやすく、
    # PR の 3 次方程式が 1 根しか返さない結果 K_i = phi_L/phi_V ≈ 1 → α ≈ 1 →
    # N_min = ∞ で全塔 infeasible になる病理がある。これを避けるため塔頂/塔底
    # それぞれの組成で泡点フラッシュを実行し、必ず 2 相が成立する点で K を取る。
    ...
    for _ in range(5):
        ...
        # _T_COND_MIN で clamp せず実温度を保持し、下流の utility_selector に
        # 適切な冷媒を選ばせる。
    ...
    # ALWAYS_CONDENSABLE_COMPS を F_top → F_bot に無条件 100% 移動すると PR EOS 実値
    # (K(C3H6) ≈ 0.18 @ -43°C/7.5bar) と整合せず、rigorous の D_total を C3=0 で固定して
    # Wang-Henke の質量保存を破綻させる。そのため閾値丸めを用いる: F_top の C3 漏れが
    # feed の _PARTIAL_COND_C3_ROUND_FRAC 未満なら 0 に丸めて塔底に集約
    # (= PSA/Mem に C3 を流さない目的を保つ)。閾値以上の漏れは物理値を残す
    # (= rigorous で D_total が物理に近づく)。
    # proxy_penalty 計算用に「丸める前の生 F_top」を保持する。閾値判定 (1%) は
    # proxy_penalty 側でも使い、丸めもこれと同じ閾値で消去する。生値を保持しておけば
    # 将来「<1% でも penalty 発火」拡張も最小変更で可能。
    F_top_pre_round = dict(F_top)
    if design.partial_condenser:
        for c in ALWAYS_CONDENSABLE_COMPS:
            moved = F_top.get(c, 0.0)
    ...
    # ---- Step 6: feasibility ----
    # FUG bias 解消のため Gilliland check を行う。(N >= N_min) かつ (R >= R_min) を
    # 「必要条件」としてだけチェックすると、narrow-margin (N ≈ N_min, R ≈ R_min) でも
    # recovery=0.99 達成可能と判定され、rigorous で大幅 spec 違反 (Dist1 HK recovery
    # 20% など) を生む。そこで Gilliland 相関で「与えられた R で recovery=0.99 達成に
    # 必要な N」を計算し、design.N_stages がそれ未満なら infeasible 化する
    # (= 十分条件チェック)。
    feasible = True
    msg = ""
    if N_min == float('inf'):
        ...
        # Gilliland (Eduljee 形) は constant α 仮定の経験式で、実際の PR EOS rigorous
        # より 10-15% 程度 conservative な N を返す傾向がある。妥当な設計 (例 Dist1:
        # N=20, R=1.5) が誤って弾かれないよう 0.85 ファクタ (= 15% tolerance) を入れる。
        # 一方 narrow-margin (ratio < 0.7) は確実に弾かれる。
    ...
    if design.partial_condenser:
        ...
        # CMO 還流は L=R×D_total なので Q_latent は R×F_top_total×λ が正しい。
        # FUG はプロファイル無しのため塔頂蒸気の顕熱は省略 (rigorous 経路は厳密
        # エネルギー収支)。残差は rigorous 比 ~10-15% 過小。
        Q_cond_kW = (design.reflux_ratio * F_top_total
                     * lam_top * 1000.0 / 3600.0)
    else:
    ...
    # 標準 MESH 式 V' = (R+1)D - (1-q)F を用いる (q に依らず物理的に正しい)。
    # q=1 (Dist1, Dist3): V' = (R+1)D。
    ...
    # 塔径は contest §4-2 仕様 (G* = SF·K·√(ρ_v(ρ_l-ρ_v))) で物理ベースに算出。
    ...
    # condenser・reboiler は独立 HE で大きな CAPEX を持ち、温度依存で utility が変わる
    # (Dist2 のように T_top 氷点下ならエチレン冷媒が必要)。そのため CAPEX に独立計上し、
    # utility_selector に T_top/T_bot を渡して適切な tier を自動選択する。
    #
