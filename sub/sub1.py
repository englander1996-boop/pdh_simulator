r"""
sub/sub1.py (旧 main.py) — PDH プロセスの多変数最適化 (Optuna ベース、FUG/rigorous)

【2026-05-29 退避】旧 main.py を sub/sub1.py へ移動。現行 BO の本丸は ../main.py (旧 special, HYSYS+SM)。
本ファイルは FUG/rigorous 全フローシート版のアーカイブ (並列対応、parallel kind='sub1')。
...
# sub/ へ退避済み。repo root は 1 階層上 (旧 main.py は repo 直下だった)。
